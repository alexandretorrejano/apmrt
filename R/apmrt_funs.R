...
fload<-function(f,...){
  ...
  if(lext=="csv"){
    d<-fread(f,integer64="character",...)
  } else if(lext=="xlsx"){
    d<-read.xlsx(f,...)%>%data.table
  } else if(lext=="xls"){
    d<-read_xls(f,...)%>%data.table
  } else if(lext=="txt"){
    d<-read.table(f,...)%>%data.table
  } else if(lext=="rds"){
    d<-readRDS(f,...)
  } else{
...
fsave<-function(d,f,...){
  ...
  if(lext=="csv"){
    ...
    d<-write.xlsx(d,f,...)%>%data.table
  } else if(lext=="txt"){
    d<-write.table(d,f,...)%>%data.table
  } else if(lext=="rds"){
    d<-saveRDS(d,f,...)
  } else{
